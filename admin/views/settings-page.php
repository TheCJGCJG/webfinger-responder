<div class="wrap">
    ...
    <form action="options.php" method="post">
        ...
        <input type="text" name="webfinger_path"
        ...
        <div id="patterns-container">
            <?php foreach ($resource_patterns as $index => $pattern_data): ?>
                <div class="pattern-entry">
                    <h3>Pattern</h3>
                    <input type="text" name="webfinger_resource_patterns[<?php echo esc_attr($index); ?>][pattern]"
                        value="<?php echo esc_attr($pattern_data['pattern']); ?>" class="regular-text">
                        ...
                        <?php foreach ($pattern_data['links'] as $link_index => $link): ?>
                            <div class="link-entry">
                                <input type="text"
                                    ...
                                    value="<?php echo esc_attr($link['rel']); ?>" placeholder="Relation (rel)"
                                    class="regular-text">
                                <input type="text"
                                    name="webfinger_resource_patterns[<?php echo esc_attr($index); ?>][links][<?php echo esc_attr($link_index); ?>][href]"
                                    value="<?php echo esc_attr($link['href']); ?>" placeholder="URL (href)"
                                    class="regular-text">
